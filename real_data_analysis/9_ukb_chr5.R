#### This R script will implement our method to fit model 2.1 on brain regional volumes (Y), genotype on chromosome 5 (X), and the environmental/demographic covariates (U).
#### The output file "chr5_result.RData" includes the estimates and the clustering patterns of the estimated coefficient vectors. 
...
options(warn = 1)
load("/mnt/project/Data/X_Rdata/chr5_X.RData")
...
save(n, d, p, q, q0, Betas, Blabels, classifications, means, covariances, cluster_nums, run_time,
     file = "chr5_result.RData")

system(paste0("dx upload chr5_result.RData"))

system("dx upload ukb_chr5.R")

system("dx terminate job-J1vPz4jJbfG0ypgQ2z4gQkJQ")
