#### This R script will implement our method to fit model 2.1 on brain regional volumes (Y), genotype on chromosome 21 (X), and the environmental/demographic covariates (U).
#### The output file "chr21_result.RData" includes the estimates and the clustering patterns of the estimated coefficient vectors. 
...
options(warn = 1)
load("/mnt/project/Data/X_Rdata/chr21_X.RData")
...
lam_0 <- opt_lam <- 8800;  lam_thred <- 7500
...
save(n, d, p, q, q0, Betas, Blabels, classifications, means, covariances, cluster_nums, run_time,
     file = "chr21_result.RData")

system(paste0("dx upload chr21_result.RData"))

system("dx upload ukb_chr21.R")

system("dx terminate job-J1qJxBjJbfGBbgp84FgXF74J")
