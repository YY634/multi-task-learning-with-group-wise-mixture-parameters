#### This R script will implement our method to fit model 2.1 on brain regional volumes (Y), genotype on chromosome 8 (X), and the environmental/demographic covariates (U).
#### The output file "chr8_result.RData" includes the estimates and the clustering patterns of the estimated coefficient vectors.  
...
options(warn = 1)
load("/mnt/project/Data/X_Rdata/chr8_X.RData")
...
lam_0 <- opt_lam <- 2300;   lam_thred <- 1900
...
save(n, d, p, q, q0, Betas, Blabels, classifications, means, covariances, cluster_nums, run_time,
     file = "chr8_result.RData")

system(paste0("dx upload chr8_result.RData"))

system("dx upload ukb_chr8.R")

system("dx terminate job-J1v7490JbfG9pPfzx4yV5B7G")
