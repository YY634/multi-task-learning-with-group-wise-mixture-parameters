#### This R script will implement our method to fit model 2.1 on brain regional volumes (Y), genotype on chromosome 13 (X), and the environmental/demographic covariates (U).
#### The output file "chr13_result.RData" includes the estimates and the clustering patterns of the estimated coefficient vectors. 
...
options(warn = 1)
load("/mnt/project/Data/X_Rdata/chr13_X.RData")
...
lam_0 <- opt_lam <- 7100;  lam_thred <- 6000
...
save(n, d, p, q, q0, Betas, Blabels, classifications, means, covariances, cluster_nums, run_time,
     file = "chr13_result.RData")

system(paste0("dx upload chr13_result.RData"))

system("dx upload ukb_chr13.R")

system("dx terminate job-J1qzfj0JbfGKXBKv2PY09Bgg")
