#### This R script will implement our method to fit model 2.1 on brain regional volumes (Y), genotype on chromosome 2 (X), and the environmental/demographic covariates (U).
#### The output includes the clustering patterns of the estimated coefficient vectors.
#### The output file "chr2_result.RData" includes the estimates and the clustering patterns of the estimated coefficient vectors.  
...
options(warn = 1)
load("/mnt/project/Data/X_Rdata/chr2_X.RData")
...
lam_0 <- opt_lam <- 5000;  lam_thred <- 4000
...
save(n, d, p, q, q0, Betas, Blabels, classifications, means, covariances, cluster_nums, run_time,
     file = "chr2_result.RData")

system(paste0("dx upload chr2_result.RData"))

system("dx upload ukb_chr2.R")

system("dx terminate job-J1vPzJ8JbfG2X83jqgg0jQ9F")
